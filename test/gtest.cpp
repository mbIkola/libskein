...
class SkeinTest : public HashShitFuncTestTemplate {
public:
    SkeinTest() : HashShitFuncTestTemplate() { digest_size = 32; /* 256 bit */  };
protected:
    string calc(string input) {
        uint8_t *input_binary = hex2bin(input);
        skein256(input_binary, input.size()/2, digest_binary);
        delete[] input_binary;
...
TEST_P(SkeinTest, defaultFixturesTest) {
    ...
    static vector<StringPair> load( const char *path_to_csv_file ) {
        ...
        while ( ! getline(infile, line).eof()) {
            if ( line[line.size()-1] == '\r')  {
                line.resize(line.size() - 1); // dirty hack against CRLF/CR/LF line ending handling in ::std::getline
            }
...
auto skeinFixtures = FixturesCSVLoader::load("data/skein.csv");
INSTANTIATE_TEST_CASE_P(defaultFixturesTest, SkeinTest, ::testing::ValuesIn(skeinFixtures));
